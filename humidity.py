...
GPIO.setup(11,GPIO.OUT)  # Set it as an output so that we can:
GPIO.output(11,GPIO.HIGH) # write a 1
time.sleep(0.025) # for 25 ms
GPIO.output(11,GPIO.LOW) # then write a 0
time.sleep(0.025) # for 20 ms.
# I assume that the preceding sequence is the method to get the DHT sensor to respond with the current data.

GPIO.setup(11, GPIO.IN, pull_up_down=GPIO.PUD_UP) # Change the pin to read mode, with a pullup resistor

for i in range(0,500): # 501 times
    data.append(GPIO.input(11)) # read a bit from the GPIO, as fast as possible (no wait)
...
except Exception, err: # if there was an error in the "try:" block
   print "ERR" # report it
